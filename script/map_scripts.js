        ...
        initialData.features.forEach(function(f) {
            if (!f.properties.director || !f.properties.production_year) {
                var meta = metadataJson.find(function(m) {
                    ...
                    var fName = (f.properties.name || f.properties.location_name || '').toLowerCase();
...
function showLocationDetails(locationName, shouldScroll = true) {
    ...
    if (metadataJson && metadataJson.length > 0) {
        locationTexts = metadataJson.find(function(elemento) {
            return elemento.location_name === locationName || (elemento.name && elemento.name.toLowerCase() === locationName.toLowerCase()); 
        });
    }
